...
//insert adds a word to the dictionary  without attaching any description
 ...
 WWW.prototype.addWord = function (txt, cue) {
	 ...
	 //so we check if cue exists, if not, donot proceed; if cue not supplied this operation assumes the cue b as the root; as root always exist , supplying only word 
	 //is same as CollectWord ; hence addWord does more than CollectWord or placeCue;
     ...
     if (c >= 0) {
         //alert("word exists" + c);
         //return -1;
		 //we next locate cue object
		 var ns = this.exists(this.metaWords.words,cue );
		 if (ns > 0 ) {
           //find cue object
		   var co =   this.metaWords.desc[this.metaWords.wid];
		  var o1 = new Object();  
		 ...
		 else { alert ("cue does not exist:" + cue); return -2 ; }
     } else {
          alert("word does not exist" + txt);
		  return -1;
    ...
    { var ii = this.findWord(w);
	 ...
	 // a old cue object suppled or cuepath supplied 
	   ...
	   // first find  word entry of cue path, locate cue object from desc
	    this.metaWords.wid = findWord(cue) ;
	   if (wid  < 0 )  {  //supplied cuepath does not exist 
	   return (-1) ;
	   } 
	   cue = this.metaWords.desc[this.metaWords.wid];
	   }
	 // now we are having a cue object
	 this.droot[w] = this.makeCue (cue,w,this.droot,this.droot,this.droot.cuepath +this.NSS + w) ;
	 this.CollectWord(this.droot.cuepath+this.NSS+w,this.droot[w]);
