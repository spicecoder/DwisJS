...
var inr = ""; 
 ...
 if ( ele.indexOf(this.cuepath + this.NSS) > -1) { return true;}
 else {return false; }
}  
    ...
    for (var property in obj) {
        ...
        if (obj.hasOwnProperty(property)) {
		   ...
		   if( !(property  in resproperty )) {
            ...
            else       {  if  (property === "it"){if (this.sMeta.exists(property )< 0){ iw  = this.sMeta.insert(obj[property]);}}
			              if  (property === "cuepath" ) {if (this.sMeta.exists(property) < 0){ isf  = this.sMeta.insert(obj[property]);
...
if (this.hasRoot(storetemp[ii],this.cuepath)) { storeCue.push(storetemp[ii]) ;  }
else {storeMe.push(storetemp[ii]);if(this.cuepath == storetemp[ii] ) {rootposition = storeMe.length -1 ;}     }
...
var W2 = new WWW(storeMe[rootposition]);
// add words to W2
for(var  jj= 0; jj<storeMe.length ; jj =jj+ 1 ){ 
W2.placeCue(storeMe[jj]);
}
...
if (elmarr.length  > 1) {  var elm = elmarr[elmarr.length - 1] ; var carr = elmarr.splice(0,elmarr.length-2); var cpaath = carr.join(this.NSS);
W2.placeCue(elm,cpaath) ;
return W2;
}
}
 }
